...
export const eleven = {
    async exec({ from, message }) {
        ...
        const mealName = message;
        const mealDetails = await getMealInformation(mealName);

        return `🍽 Here’s some information about "${mealName}":\n\n${mealDetails}\n\nInteresting, right? 😊`;
    }
};


async function getMealInformation(mealName) {
    const payload = {
        "prompt":  `Describe the origin, history, and popularity of the meal: ${mealName}.`
    };
