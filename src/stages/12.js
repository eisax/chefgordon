...
export const twelve = {
    async exec({ from, message }) {
        ...
        const timeOfDay = currentHour < 12 ? 'breakfast' : currentHour < 17 ? 'lunch' : 'dinner';


        return `🕒 Based on the time of day (${timeOfDay}), here are some suggestions:\n\n` +
            (await suggestMealsByTime(timeOfDay)).join('\n') +
            '\n\nLet me know if you’d like a recipe for any of these! 🍴';
    }
};


async function suggestMealsByTime(timeOfDay) {
    const payload = {
        "prompt": `Suggest three ${timeOfDay} meals.`
    };
