...
export const ten = {
    async exec({ from, message }) {
        ...
        const query = message;
        const nutritionInfo = await getNutritionalInformation(query);

        return `🍎 Here’s the nutritional information for "${query}":\n\n${nutritionInfo}\n\nStay healthy! 💪`;
    }
};


async function getNutritionalInformation(query) {
    const payload = {
        "prompt": `Provide the nutritional information for: ${query}.`
    };
