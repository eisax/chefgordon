...
export const six = {
    async exec({ from, message }) {
        ...
        const topic = message;
        const tips = await getCookingTips(topic);

        return `🧑‍🍳 Here are some tips for ${topic}:\n\n${tips}\n\nLet me know if you need further assistance!`;
    }
};


async function getCookingTips(leftovers) {
    const payload = {
        "prompt": `Suggest three creative dishes using these leftovers: ${leftovers}.`
    };
